...
public class ConversationModel extends RepresentationModel<ConversationModel> {
  ...
  public ConversationModel(User user, Conversation conversation, UriInfo uriInfo) {
    ...
    add(Link.of(ApiTemplates.conversation(uriInfo).build(user.getIdentity(), conversation.getIdentity()).getPath(), "self").withType(HttpMethod.GET));
    add(Link.of(ApiTemplates.messages(uriInfo).build(user.getIdentity(), conversation.getIdentity()).getPath(),
