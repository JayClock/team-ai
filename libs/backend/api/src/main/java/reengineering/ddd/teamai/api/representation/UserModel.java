...
public class UserModel extends RepresentationModel<UserModel> {
  ...
  public UserModel(User user, UriInfo uriInfo) {
    ...
    Link accountsRel = Link.of(ApiTemplates.accounts(uriInfo).build(user.getIdentity()).getPath())
        .withRel("accounts");
    Link conversationsRel = Link.of(ApiTemplates.conversations(uriInfo).build(user.getIdentity()).getPath())
