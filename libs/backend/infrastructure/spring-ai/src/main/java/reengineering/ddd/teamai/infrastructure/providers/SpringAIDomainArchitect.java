...
public class SpringAIDomainArchitect implements Diagram.DomainArchitect, RequestHeaderModelConfig {
  ...
  private String buildAnalysisPrompt(String requirement, String outputFormat) {
    ...
    return """
                ...
                2. Party 角色化：每个 Participant（Party) 必须生成一个对应的 Party Role，并在后续连线中体现 Participant（Party) -> Party Role -> Contract。
                ...
                6. 角色拆分总则：按业务意图补齐 Role；Party Role、Evidence As Role 及 Third/Context 参与边界分别以第 2、9、10 条为准。
                7. 参与方判定：新增 RFP、Proposal、Fulfillment Request、Fulfillment Confirmation、Other Evidence 时，必须先思考并明确唯一的 Party Role 参与再连线；上述每个节点只能有一个 Party Role 参与。
                ...
                6. Context 包裹规则：当节点 type=CONTEXT 时必须作为容器父节点，并包裹本上下文从 RFP 到终点节点（Other Evidence 或 Evidence As Role）的全部相关节点（含对应 Request/Confirmation/Role/Participant）；当出现 Evidence As Role 时，可将其作为上下文划分边界标记。Evidence As Role 的 parent.id 必须与其“原始 Other Evidence 的生成来源 Fulfillment Confirmation”保持一致（同 Context）；若后续还连接了其他 Fulfillment Confirmation，也不得改变该归属。
                ...
                8. 角色连接规则：
                   ...
                   - Third Party Role、Context Role 的参与范围遵循「流程建模第 10 条」约束。
