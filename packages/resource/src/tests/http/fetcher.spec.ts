...
describe('Fetcher', () => {
  ...
  describe('fetch', () => {
    ...
    it('should add User-Agent header when config.sendUserAgent is true', async () => {
    ...
    it('should not add User-Agent header when config.sendUserAgent is false', async () => {
      fetcher = new Fetcher({ ...mockConfig, sendUserAgent: false });
      await fetcher.fetch('https://api.example.com/test');

      const calledRequest = mockFetch.mock.calls[0][0];
      expect(calledRequest.headers.has('User-Agent')).toBe(false);
    });
    ...
    it('should not override existing User-Agent header', async () => {
      ...
      await fetcher.fetch('https://api.example.com/test', init);

      const calledRequest = mockFetch.mock.calls[0][0];
      expect(calledRequest.headers.get('User-Agent')).toBe('Custom-Agent');
    });

    it('should call middlewares in sequence', async () => {
      const middleware1 = vi.fn((req, next) => {
        req.headers.set('X-Middleware-1', 'true');
        return next(req);
      });
      const middleware2 = vi.fn((req, next) => {
        req.headers.set('X-Middleware-2', 'true');
        return next(req);
      });

      fetcher.use(middleware1, 'https://api.example.com');
      fetcher.use(middleware2, 'https://api.example.com');
      ...
      const calledRequest = mockFetch.mock.calls[0][0];
      expect(calledRequest.headers.get('X-Middleware-1')).toBe('true');
      expect(calledRequest.headers.get('X-Middleware-2')).toBe('true');
    });

    it('should only call middlewares that match the origin', async () => {
      const middleware1 = vi.fn();
      const middleware2 = vi.fn();

      fetcher.use(middleware1, 'https://api.example.com');
      fetcher.use(middleware2, 'https://other.com');
